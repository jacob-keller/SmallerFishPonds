...
{
    public class SmallPondFishSilhouette : PondFishSilhouette
    {
        public float border_width = 0.8f;
        public new SmallerFishPond _pond;

        /// <summary>
        /// Adjusted initial shadow placement
        /// </summary>
        /// <param name="pond"></param>
        public SmallPondFishSilhouette(SmallerFishPond pond, float bWidth)
            : base(pond)
        {
            _pond = pond;
            ...
            position = new Vector2(
                _pond.tileX.Value + Utility.Lerp(border_width + 0.2f, (float)_pond.tilesWide.Value - border_width - 0.2f, (float)Game1.random.NextDouble()),
                _pond.tileY.Value + Utility.Lerp(border_width + 0.2f, (float)_pond.tilesHigh.Value - border_width - 0.2f, (float)Game1.random.NextDouble())
                ) * 64f;
            _age = 0f;
        ...
        public new void Update(float time)
        {
            ...
            {
                position.X = ((float)(_pond.tileX.Value + _pond.tilesWide.Value) - border_width) * 64f -1f;
                _velocity.X *= -1f;
            ...
            {
                position.X = ((float)_pond.tileX.Value + border_width) * 64f + 1f;
                _velocity.X *= -1f;
                if (moving && (Game1.random.NextDouble() < 0.25 || Math.Abs(_velocity.X) > 30f))
                {
                    _flipped = !_flipped;
            ...
            {
                position.Y = ((float)(_pond.tileY.Value + _pond.tilesHigh.Value) - border_width) * 64f - 1f;
                _velocity.Y *= -1f;
            }
            if (position.Y < ((float)_pond.tileY.Value + border_width) * 64f)
            {
                position.Y = ((float)_pond.tileY.Value + border_width) * 64f + 1f;
                _velocity.Y *= -1f;
            }
        }
    }
